...
Estudantes de 15--17 anos da rede pública em situação de pobreza (renda per capita $\le$ 0,5 SM) & $\sim$60\% & Brasil & \citeonline{fadc2023} \\
...
\begin{description}
    \item[H1.] Classificadores de \textit{ensemble} (Floresta Aleatória e XGBoost) superam a árvore de decisão única e os \textit{baselines} de referência (classificador majoritário e Regressão Logística) na predição da tendência de desempenho.
...
O presente trabalho propõe, portanto, a construção de um conjunto de dados sintéticos sociologicamente fundamentado e o desenvolvimento comparativo de modelos de classificação (Árvore de Decisão, Floresta Aleatória e XGBoost) para prever a tendência de desempenho ao final do ano letivo (Aprovado, Em Risco ou Reprovado), a partir de sinais acadêmicos parciais do primeiro semestre e do contexto socioeconômico, identificando as variáveis mais determinantes e fornecendo subsídios para intervenções pedagógicas preventivas.
